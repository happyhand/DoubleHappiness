...
namespace DataInfo.Api.Controllers.Interactive
{
    ...
    public class FriendController : JwtController
    {
        ...
        public async Task<IActionResult> Delete(string targetID)
        {
            ...
            try
            {
                this.logger.LogInfo("會員請求移除好友", $"MemberID: {memberID} TargetID: {targetID}", null);
                ...
                ResponseResult responseResult = await this.InteractiveService.UpdateInteractive(new InteractiveContent() { MemberID = targetID }, memberID, InteractiveType.Friend, ActionType.Delete).ConfigureAwait(false);
                return this.ResponseHandler(responseResult);
            }
            catch (Exception ex)
            {
                this.logger.LogError("會員請求移除好友發生錯誤", $"MemberID: {memberID} TargetID: {targetID}", ex);
                return this.ResponseHandler(new ResponseResult()
                {
                    Result = false,
